...
(
	...
	insert into `vehicle` values('VE1001','布加迪威龙','青岛','RU1001','2018-08-25',now(),'');
	select * from `vehicle`;


# 配送点 drop table distribution
create table if not exists `distribution`
(
	dis_id int not null primary key comment'配送点id(主键)',
	dis_name nvarchar(10) comment'配送点名称',
	pro_id nvarchar(10) references province(pro_id),
	cit_id nvarchar(10) references city(cit_id),
	adm_id nvarchar(10) references admin(adm_id),
	dis_phone nvarchar(20) comment'联系电话',
	dis_address nvarchar(50) comment'地址',
	dis_remarks nvarchar(50) comment'备注'
);

	insert into `distribution` values(100924,'哈哈地点','S1002','C1002','U1001','13897265432','D栋25','');
	select * from `distribution`;

#配送范围 drop table distribution_scope
...
(
	...
	insert into `distribution_scope` values(387653,'呵呵地点',14,'C1002','');
	select * from `distribution_scope`;
